...
\oldc{Podemos usar o rastreamento de olhar para construir interfaces para ajudar pessoas com dificuldades motoras a interagir com computadores \cite{lidade}. Informações de olhar também podem ser usadas em pesquisas com o objetivo de entender como o olho se move quando uma pessoa realiza diferentes atividades \cite{lupung}.}

\newc{Abaixo listamos algumas aplicações de rastreamento de olhar.}

\section{Aplicações}

\newc{Podemos usar o rastreamento de olhar para construir interfaces para ajudar pessoas com dificuldades motoras a interagir com computadores \cite{lidade}. \cite{kurauchi2016eyeswipe} desenvolveu um teclado virtual que permite a digitação através do olhar, como mostra a Figura $\ref{fig:eyeswipe}$.}
...
Através da análise da posição do olhar é possível avaliar a influência de um anúncio sobre a atenção dos consumidores e, assim, ajudar na criação de propagandas mais eficientes \cite{duchowski2002breadth}. \newc{A Figura \ref{fig:propaganda} mostra as regiões onde as pessoas mais prestam atenção ao observar um anúncio.}

\afterpage{
\begin{figure}
\begin{center}
\includegraphics[scale=.4]{imagens/dolcegabbana.jpg}
\caption[]{O rastreamento de olhar pode ser usado para a avaliar a eficiência de uma propaganda. Regiões coloridas indicam onde as pessoas prestam mais atenção no anúncio. Reproduzido de \protect \footnotemark}
...
\newc{Técnicas de rastreamento de olhar determinam a posição do olhar a partir do movimento dos olhos}. Algumas partes do olho humano, que estão representadas na Figura $\ref{fig:olho}$,  são as seguintes \cite{lidade}:
...
\item \oldc{Instalando algum dispositivo mecânico sobre uma lente de contato.} \newc{{\bf Lente de contato}, uma bobina é instalada no olho sobre uma lente de contato e a posição do olho é estimada ao medir o campo eletromagnético. \cite{duchowski2007eye}. Este método é o mais preciso para medir movimentos do olho, porém é o mais invasivo \cite{duchowski2007eye}.}
...
O rastreamento baseado em vídeo \newc{é} a modalidade de rastreamento de olhar menos invasiva \cite{valenti2009webcam}.

Dois tipos de imagem \oldc{podem ser}\newc{são comumente} usados no rastramento baseado em vídeo: imagens no espectro visual ou imagens em infravermelho.
...
No caso de imagens em infravermelho, não temos esse problema, pois o olho é constantemente iluminado por uma fonte de luz infravermelha, que não é percebida pelo usuário. Uma vantagem desta abordagem é que a pupila reflete \oldc{boa} parte da luz infravermelha recebida, sendo a região mais brilhante do olho na imagem $\cite{lidade}$. Devido \oldc{ao tamanho} \newc{ao seu tamanho e posição}, a pupila tem menor chance de ser parcialmente ocultada pelos  cílios do que a esclera, outra região que reflete o infravermelho. %pupila será sempre a região mais brilhante na imagem $\cite{lupung}$: se a fonte de luz estiver alinhada com o olho, a pupila aparecerá branca, caso contrário, estará preta na imagem.
...
Uma vantagem do rastreamento \textit{head-mounted} é que a câmera \newc{se} move com a cabeça, então mudanças de pose não causam mudanças da posição da pupila na imagem. A desvantagem é a necessidade de usar um equipamento acoplado à cabeça. Por estar muito próxima do olho, a câmera do olho pode ocultar parte do campo de visão, dificultando a interação com o usuário.

\section{\newc{Características em} rastreamento de olhar}

Em algumas aplicações de rastreamento de olhar não estamos interessados apenas na direção do olhar em determinado instante, mas também \oldc{poderemos} \newc{podemos} querer observar outras características \oldc{(métricas)} relacionadas ao movimento do olhos. As \oldc{métricas}\newc{características} mais específicas são \cite{lupung}:
