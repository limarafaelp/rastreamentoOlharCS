...
\begin{enumerate}
\item os elementos $a_{ij}$ de A \oldc{são iid e $a_{ij} \sim \mathcal{N}(0,1)$}\newc{são independentes e têm distribuição normal com média $0$ e variância $1$.}
\item $A$ é uma matriz de posto $m$ com $BA^{T} = 0$ para uma matriz $B \in \mathbb{R}^{(n - m) \times n}$ cujas entradas são \newc{independentes e têm distribuição normal com média $0$ e variância $1$.}\oldc{iid $\mathcal{N}(0,1)$.}
\end{enumerate}
